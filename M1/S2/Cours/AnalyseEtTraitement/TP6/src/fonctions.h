...
//Pré traitement carte de couleur

void colormap_to_identifiermap(ImageBase &inColorMap , ImageBase &outMap , int birthday_n){
    //if all black, then 0
    //if index == birthday n -> all white

    int numCol = 0;
    std::vector<std::vector<int>> visited_colors;
    
    for (int x = 1; x < inColorMap.getHeight() - 1; x++) {
        for (int y = 1; y < inColorMap.getWidth() - 1; y++) {

            //get the colors
            int colorR = inColorMap[x*3][y*3+0];
            int colorG = inColorMap[x*3][y*3+1];
            int colorB = inColorMap[x*3][y*3+2];

            std::vector<int> color = {colorR, colorG, colorB};

            //black and white check
            bool isBlack = (colorR == 0 && colorG == 0 && colorB == 0);
            if(isBlack) {
                outMap[x][y] = 0;
                continue;
            }

            //other colors find a way to associate a unique grey val for each color
            auto it = std::find(visited_colors.begin(), visited_colors.end(), color);
            int val = 0;
            if (it != visited_colors.end()) {
                int index = std::distance(visited_colors.begin(), it);
                val = index + 1;
            } else {
                visited_colors.push_back(color);
                val = numCol++;
            }
...
void synthesize_avg(ImageBase &imIn , ImageBase &identifierMap , ImageBase &imOut){

    //get number of regions
    int maxRegion = 0;
    for (int x = 0; x < identifierMap.getHeight(); x++) {
        for (int y = 0; y < identifierMap.getWidth(); y++) {
            if (identifierMap[x][y] > maxRegion && identifierMap[x][y] != 255) {
                maxRegion = identifierMap[x][y];
            }
        }
    }
    std::cout << "max regions : " << maxRegion << std::endl;

    //calculate avg/region
    std::vector<int> regions(maxRegion + 1 , 0);
    std::vector<int> sumsPerRegion(maxRegion + 1, 0);
    std::vector<int> numValPerRegion(maxRegion + 1, 0);

    for (int x = 0; x < imIn.getHeight(); x++) {
        for (int y = 0; y < imIn.getWidth(); y++) {
            int region = identifierMap[x][y];
            if (region != 255) {
                sumsPerRegion[region] += imIn[x][y];
                numValPerRegion[region]++;
            }
        }
    }

    for (int i = 0; i < regions.size(); i++) {
        if (numValPerRegion[i] > 0) {
            regions[i] = sumsPerRegion[i] / numValPerRegion[i];
        }
    }

    ImageBase imAvg(identifierMap.getWidth() , identifierMap.getHeight() , identifierMap.getColor());

    for (int x = 0; x < imIn.getHeight(); x++) {
        for (int y = 0; y < imIn.getWidth(); y++) {
            int region = identifierMap[x][y];
            if (region != 255) {
                imAvg[x][y] = regions[region];
            } else{
                imAvg[x][y] = regions[12];
            }
        }
    }

    imAvg.save("img/out/avg.pgm");

    //shuffle the average to get a cool region map
    ImageBase imRand(identifierMap.getWidth() , identifierMap.getHeight() , identifierMap.getColor());

    std::vector<int> shuffledRegions = regions;
    std::cout << "shuffled regions : " << shuffledRegions.size() << std::endl;
    
    std::default_random_engine rng(24052003);
    std::shuffle(shuffledRegions.begin(), shuffledRegions.end(), rng);

    for (int x = 0; x < imIn.getHeight(); x++) {
        for (int y = 0; y < imIn.getWidth(); y++) {
            int region = identifierMap[x][y];
            if (region != 0) {
                imRand[x][y] = shuffledRegions[region];
            }
        }
    }

    imRand.save("img/out/shuffled.pgm");

    //make the final image
    for (int x = 0; x < imIn.getHeight(); x++) {
        for (int y = 0; y < imIn.getWidth(); y++) {
            if (identifierMap[x][y]) {
                imOut[x][y] = imIn[x][y] - imAvg[x][y] + imRand[x][y];
            }
        }
    }    
}

void synthesize_variance(ImageBase &imIn, ImageBase &identifierMap, ImageBase &imOut) {
    // Get number of regions
    int maxRegion = 0;
    for (int x = 0; x < identifierMap.getHeight(); x++) {
        for (int y = 0; y < identifierMap.getWidth(); y++) {
    ...
    std::vector<int> sumsPerRegion(maxRegion + 1, 0);
    std::vector<int> numValPerRegion(maxRegion + 1, 0);
    std::vector<int> sumSquaresPerRegion(maxRegion + 1, 0);
    ...
    for (int x = 0; x < imIn.getHeight(); x++) {
        for (int y = 0; y < imIn.getWidth(); y++) {
            ...
            if (region != 255) {
                int pixel = imIn[x][y];
                sumsPerRegion[region] += pixel;
                sumSquaresPerRegion[region] += pixel * pixel;
                numValPerRegion[region]++;
            }
        }
    }
    ...
    for (int i = 0; i < regions.size(); i++) {
        if (numValPerRegion[i] > 0) {
            int mean = sumsPerRegion[i] / numValPerRegion[i];
            int meanSquare = sumSquaresPerRegion[i] / numValPerRegion[i];
            regions[i] = sqrt(meanSquare - mean * mean);
        }
    }

    ImageBase imVariance(identifierMap.getWidth(), identifierMap.getHeight(), identifierMap.getColor());

    for (int x = 0; x < imIn.getHeight(); x++) {
        for (int y = 0; y < imIn.getWidth(); y++) {
            int region = identifierMap[x][y];
            if (region != 255) {
                imVariance[x][y] = regions[region];
            } else {
                imVariance[x][y] = regions[12];
            }
        }
    }

    imVariance.save("img/out/variance.pgm");

    // Shuffle the variance to get a cool region map
    ImageBase imRand(identifierMap.getWidth(), identifierMap.getHeight(), identifierMap.getColor());
    ...
    imRand.save("img/out/shuffled_variance.pgm");

    // Make the final image
    for (int x = 0; x < imIn.getHeight(); x++) {
        for (int y = 0; y < imIn.getWidth(); y++) {
            if (identifierMap[x][y]) {
                imOut[x][y] = imIn[x][y] - imVariance[x][y] + imRand[x][y];
            }
        }
    }
}
