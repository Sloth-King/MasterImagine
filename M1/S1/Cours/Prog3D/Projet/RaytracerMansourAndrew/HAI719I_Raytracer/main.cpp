...
void ray_trace_from_camera() {
    ...
    for (int y=0; y<h; y++){
        std::clog << "\r\tScanlines remaining: " << (h-y) << ' ' << std::flush; //merci tom
        for (int x=0; x<w; x++) {
            for( unsigned int s = 0 ; s < nsamples ; ++s ) {
                float u = ((float)(x) + (float)(rand())/(float)(RAND_MAX)) / w;
                float v = ((float)(y) + (float)(rand())/(float)(RAND_MAX)) / h;
                float time = (float)(rand()) / float(RAND_MAX); //normally between 0 and 1
                // this is a random uv that belongs to the pixel xy.
                screen_space_to_world_space_ray(u,v,pos,dir);
                //Vec3 color = scenes[selected_scene].rayTrace( Ray(pos , dir) ); //no blur
                Ray ray(pos,dir,time);
                Vec3 color = scenes[selected_scene].rayTrace(ray); //blur
                image[x + y*w] += color;
            }
