...
struct Mesh {
    ...
    void initBuffers(){
        ...
        glGenBuffers(1,&colorbuffer);
        glBindBuffer(GL_ARRAY_BUFFER , colorbuffer);
        glBufferData(GL_ARRAY_BUFFER , normals.size() * sizeof(Vec3) , &normals[0] , GL_STREAM_DRAW);
        glVertexAttribPointer(1, 3 , GL_FLOAT , GL_FALSE , 0 , (void*)0);
...
void draw () {
    ...
    /*first_triangle.draw();
    int scalePosition = glGetUniformLocation(programID , "scale");
    glUniform1f(scalePosition,scale);
    int translatePosition = glGetUniformLocation(programID,"translate");
    glUniform3f(translatePosition,translate[0] ,translate[1],translate[2] );*/
