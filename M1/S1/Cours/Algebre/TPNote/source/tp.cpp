...
public:
    ...
    Transform(Mat3 i_transformation = Mat3::Identity(), Vec3 i_translation = Vec3(0., 0., 0.))
        ...
        Mat3 inv = i_transformation.inverse(i_transformation);
        inv.transpose();
        m_vector_transformation = inv;
    }
...
Vec3 project(Vec3 const &input_point, Plane const &i_plane) {
    ...
    float distance = Vec3::dot((input_point - i_plane.point), i_plane.normal);
    result -= distance * i_plane.normal;
    return result;
}
...
Vec3 project(Vec3 const &input_point, Vec3 const &i_origin, Vec3 const &i_axis) {
    Vec3 v = input_point - i_origin;
    float t = (v.dot(v,i_axis)) / (i_axis.dot(v,i_axis)); 
    Vec3 proj_vector = t * i_axis; 
    Vec3 result = i_origin + proj_vector; 
    return result; 
}
...
int main(int argc, char **argv) {
    ...
    //Mat3 transformation = 2 * scale_matrix;
    Mat3 transformation = Mat3::getFromCols(Vec3(1., 0., 0.), Vec3(0., 1., 0.), Vec3(1., 1., 1.));
