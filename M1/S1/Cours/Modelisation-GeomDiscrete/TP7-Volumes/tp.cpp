...
struct Mesh
{
    ...
    // Ajoutes ici pour simplifier l'affichage
    std::vector<Vec3> cube; // points du cube
    Grid grid;              // grid du cube
...
void Mesh::simplify(unsigned int resolution)
{
    ...
    float cubeSize = std::max(maxx - minx, std::max(maxy - miny, maxz - minz));
    Vec3 center = Vec3((minx + maxx) / 2.0f, (miny + maxy) / 2.0f, (minz + maxz) / 2.0f);

    float halfCubeSize = cubeSize / 2.0f;
    cube.clear();
    cube.push_back(center + Vec3(-halfCubeSize, -halfCubeSize, -halfCubeSize));
    cube.push_back(center + Vec3(halfCubeSize, -halfCubeSize, -halfCubeSize));
    cube.push_back(center + Vec3(halfCubeSize, halfCubeSize, -halfCubeSize));
    cube.push_back(center + Vec3(-halfCubeSize, halfCubeSize, -halfCubeSize));
    cube.push_back(center + Vec3(-halfCubeSize, -halfCubeSize, halfCubeSize));
    ...
    for (size_t i = 0; i < vertices.size(); i++)
    {
        ...
        grid.cells[index].pos += pos;
        grid.cells[index].normal += normal;
        grid.cells[index].count += 1;
    }
    ...
    {
        int index0 = grid.getIndex(vertices[triangles[i][0]]);
        int index1 = grid.getIndex(vertices[triangles[i][1]]);
        int index2 = grid.getIndex(vertices[triangles[i][2]]);
    ...
    for (size_t i = 0; i < vertices.size(); i++)
    {
        ...
        Vec3 normal = normals[i];
        int index = grid.getIndex(pos);
        pos = grid.cells[index].pos / grid.cells[index].count;
        normal = grid.cells[index].normal / grid.cells[index].count;
        normal.normalize();
...
{ // one-ring of each vertex, i.e. a list of vertices with which it shares an edge
    ...
    for (auto &t : i_triangles)
    {
        ...
        unsigned int i1 = t[1];
        unsigned int i2 = t[2];
        if (std::find(o_one_ring[i0].begin(), o_one_ring[i0].end(), i1) == o_one_ring[i0].end())
            o_one_ring[i0].push_back(i1);
        if (std::find(o_one_ring[i0].begin(), o_one_ring[i0].end(), i2) == o_one_ring[i0].end())
            o_one_ring[i0].push_back(i2);

        if (std::find(o_one_ring[i1].begin(), o_one_ring[i1].end(), i0) == o_one_ring[i1].end())
            o_one_ring[i1].push_back(i0);
        if (std::find(o_one_ring[i1].begin(), o_one_ring[i1].end(), i2) == o_one_ring[i1].end())
            o_one_ring[i1].push_back(i2);

        if (std::find(o_one_ring[i2].begin(), o_one_ring[i2].end(), i0) == o_one_ring[i2].end())
            o_one_ring[i2].push_back(i0);
        if (std::find(o_one_ring[i2].begin(), o_one_ring[i2].end(), i1) == o_one_ring[i2].end())
            o_one_ring[i2].push_back(i1);
    }
}
...
void drawMesh(Mesh const &i_mesh, bool draw_field = false)
{
    ...
    for (const Vec3 &point : mesh.cube)
    {
...
void drawPointMesh(Mesh const &i_mesh)
{
    glBegin(GL_POINTS);
    for (int i = 0; i < i_mesh.vertices.size(); i++)
    {
        Vec3 p = i_mesh.vertices[i];
        Vec3 n = i_mesh.normals[i];
...
void drawGrid(const Grid &grid)
{
    ...
    glColor3f(0.f, 0.f, 0.0f); // Set line color to green
    ...
    for (int x = 0; x < grid.resolution; ++x)
    {
        for (int y = 0; y < grid.resolution; ++y)
        {
            ...
            {
                Vec3 minPos = grid.minPos;
                Vec3 maxPos = grid.maxPos;
                float stepX = (maxPos[0] - minPos[0]) / grid.resolution;
                float stepY = (maxPos[1] - minPos[1]) / grid.resolution;
                float stepZ = (maxPos[2] - minPos[2]) / grid.resolution;

                Vec3 start = minPos + Vec3(x * stepX, y * stepY, z * stepZ);
                Vec3 endX = start + Vec3(stepX, 0, 0);
                Vec3 endY = start + Vec3(0, stepY, 0);
                Vec3 endZ = start + Vec3(0, 0, stepZ);

                glVertex3f(start[0], start[1], start[2]);
                glVertex3f(endX[0], endX[1], endX[2]);

                glVertex3f(start[0], start[1], start[2]);
                glVertex3f(endY[0], endY[1], endY[2]);

                glVertex3f(start[0], start[1], start[2]);
                glVertex3f(endZ[0], endZ[1], endZ[2]);
            }
        }
    }
    glEnd();
...
void displayVoxel(Vec3 center , double length){
    double halfVoxel = length/2.0;

    glBegin(GL_QUADS);

    glVertex3f(center[0] - halfVoxel, center[1] - halfVoxel, center[2] + halfVoxel);
    glVertex3f(center[0] + halfVoxel, center[1] - halfVoxel, center[2] + halfVoxel);
    glVertex3f(center[0] + halfVoxel, center[1] + halfVoxel, center[2] + halfVoxel);
    glVertex3f(center[0] - halfVoxel, center[1] + halfVoxel, center[2] + halfVoxel);
    ...
    //here to remember all the combinations for the corners of a cube
    // cube.push_back(center + Vec3(-halfCubeSize, -halfCubeSize, -halfCubeSize));
    // cube.push_back(center + Vec3(halfCubeSize, -halfCubeSize, -halfCubeSize));
    // cube.push_back(center + Vec3(halfCubeSize, halfCubeSize, -halfCubeSize));
    // cube.push_back(center + Vec3(-halfCubeSize, halfCubeSize, -halfCubeSize));
...
void displayVolumicSphere(Vec3 center , double radius , int resolution){

    // 1. create voxel grid 
    //use the same logic as simplify (its the same thing.)
    Vec3 minPos = center - Vec3(radius, radius, radius);
    Vec3 maxPos = center + Vec3(radius, radius, radius);
    ...
    // 2. draw the sphere
    //f(p) = p - c, if < r, display else dont
    ...
    for (int x = 0; x < grid.resolution; ++x)
    {
        for (int y = 0; y < grid.resolution; ++y)
        {
            for (int z = 0; z < grid.resolution; ++z)
            {
                //Vec3 voxCenter = minPos + Vec3(x * stepX, y * stepY, z * stepZ); //prend le coin d'une voxel
                Vec3 voxCenter = minPos + Vec3((x + 0.5f) * stepX, (y + 0.5f) * stepY, (z + 0.5f) * stepZ); //la c'est bieng
                float distance = (voxCenter - center).length();

                if (distance < radius)
                {
                    displayVoxel(voxCenter, stepX);
                }
            }
        }
    }
...
void displayVolumicCylinder(Vec3 originAxis, Vec3 axisVector, double radius, int resolution) {

    Grid grid;

    Vec3 minPos = originAxis - Vec3(radius, radius, radius);
    Vec3 maxPos = originAxis + axisVector + Vec3(radius, radius, radius);
    ...
    for (int x = 0; x < grid.resolution; ++x) {
        for (int y = 0; y < grid.resolution; ++y) {
            for (int z = 0; z < grid.resolution; ++z) {
                Vec3 voxCenter = minPos + Vec3((x + 0.5f) * stepX, (y + 0.5f) * stepY, (z + 0.5f) * stepZ);
                Vec3 projection = originAxis + ((Vec3::dot(voxCenter - originAxis, axisVector) / axisVector.squareLength()) * axisVector);

                Vec3 dir = axisVector;
                dir.normalize();

                float distAxis = (voxCenter - projection).length();
                float proj = Vec3::dot((projection - originAxis),dir);

        
                if (distAxis < radius && proj >= 0 && proj <= axisVector.length()) {
                    displayVoxel(voxCenter , stepX);
                }
            }
        }
...
Vec3 minVec(Vec3 u , Vec3 v){
    float minx = std::min(u[0],v[0]);
    float miny = std::min(u[1],v[1]);
    float minz = std::min(u[2],v[2]);

    return Vec3(minx , miny , minz);
}

Vec3 maxVec(Vec3 u , Vec3 v){
    float maxx = std::max(u[0],v[0]);
    float maxy = std::max(u[1],v[1]);
    float maxz = std::max(u[2],v[2]);

    return Vec3(maxx , maxy , maxz);
}

void displayIntersectionSphereCylinder(Vec3 sphereCenter , double sphereRadius , Vec3 cylinderOriginAxis , Vec3 cylinderVectorAxis , double cylinderRadius , double resolution){

    Grid grid;

    Vec3 circleMinPos = sphereCenter - Vec3(sphereRadius, sphereRadius, sphereRadius);
    Vec3 circleMaxPos = sphereCenter + Vec3(sphereRadius, sphereRadius, sphereRadius);
    
    Vec3 cylinderMinPos = cylinderOriginAxis - Vec3(cylinderRadius , cylinderRadius , cylinderRadius);
    Vec3 cylinderMaxPos = cylinderOriginAxis + cylinderVectorAxis + Vec3(cylinderRadius , cylinderRadius , cylinderRadius);

    Vec3 minPos = minVec(circleMinPos , cylinderMinPos);
    Vec3 maxPos = maxVec(circleMaxPos , cylinderMaxPos);
    ...
    for (int x = 0; x < grid.resolution; ++x) {
        for (int y = 0; y < grid.resolution; ++y) {
            for (int z = 0; z < grid.resolution; ++z) {
                ...
                float sphereDistance = (voxCenter - sphereCenter).length();

                bool insideSphere = false;
                if(sphereDistance < sphereRadius) insideSphere = true;
    
                Vec3 projection = cylinderOriginAxis + ((Vec3::dot(voxCenter - cylinderOriginAxis, cylinderVectorAxis) / cylinderVectorAxis.squareLength()) * cylinderVectorAxis);
                Vec3 dir = cylinderVectorAxis;
                dir.normalize();

                float distAxis = (voxCenter - projection).length();
                float proj = Vec3::dot((projection - cylinderOriginAxis) , dir);

                bool insideCylinder = false;
                if((distAxis < cylinderRadius) && (proj >= 0) && (proj <= cylinderVectorAxis.length())){
                    insideCylinder = true;
                }
...
void displayUnionSphereCylinder(Vec3 sphereCenter , double sphereRadius , Vec3 cylinderOriginAxis , Vec3 cylinderVectorAxis , double cylinderRadius , double resolution){
...
void displaySubstractSphereCylinder(Vec3 sphereCenter , double sphereRadius , Vec3 cylinderOriginAxis , Vec3 cylinderVectorAxis , double cylinderRadius , double resolution){
    ...
    Vec3 cylinderMinPos = cylinderOriginAxis - Vec3(cylinderRadius , cylinderRadius , cylinderRadius);
    Vec3 cylinderMaxPos = cylinderOriginAxis + Vec3(cylinderRadius , cylinderRadius , cylinderRadius);
...
void draw()
{
    ...
    glColor3f(0.02, 0.58, 0.58);
    // drawMesh(mesh, true);
    // drawPointMesh(mesh);
    //drawGrid(mesh.grid);
    //displayVoxel(Vec3(0.f,0.f,0.f) , 0.5);
    //displayVolumicSphere(Vec3(0 , 0 , 0) , 0.5 , 15);
    //displayVolumicCylinder(Vec3(0,0,0) , Vec3(0.5,0.5,0.5) , 0.2 , 25);
    displayIntersectionSphereCylinder(Vec3(0 , 0 , 0) , 0.5 , Vec3(0,0,0) , Vec3(0.5,0.5,0.5) , 0.2 , 25);
    // drawCubeCorners();
...
void displayScene1(){
    glColor3f(0.0, 0.40, 0.31);
    displayVolumicSphere(Vec3(0, 0, 0), 0.5, res);
}
void displayScene2(){
    displayVolumicCylinder(Vec3(0, 0, 0), Vec3(0.5, 0.5, 0.5), 0.2, res);
}

void displayScene3(){
    displayIntersectionSphereCylinder(Vec3(0, 0, 0), 0.5, Vec3(0, 0, 0), Vec3(0.5, 0.5, 0.5), 0.2, res);
}
void displayScene4(){
    displayUnionSphereCylinder(Vec3(0, 0, 0), 0.5, Vec3(0, 0, 0), Vec3(0.5, 0.5, 0.5), 0.2, res);
}
void displayScene5(){
    displaySubstractSphereCylinder(Vec3(0, 0, 0), 0.5, Vec3(0, 0, 0), Vec3(0.5, 0.5, 0.5), 0.2, res);
}
void displayScene6(){
    displayVoxel(Vec3(0.f,0.f,0.f) , 0.5);
}
...
void key(unsigned char keyPressed, int x, int y)
{
    ...
    case '2': // Simplification by grid
        std::cout << "original mesh vertices : " << originalMesh.vertices[0] << std::endl;
        mesh = originalMesh;
        mesh.vertices = originalMesh.vertices;
        std::cout << "mesh : " << mesh.vertices[0] << std::endl;
...
int main(int argc, char **argv)
{
    ...
    std::cout << "openoff " << originalMesh.vertices.size() << std::endl;
